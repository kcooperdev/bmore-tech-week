...
const EventCard: React.FC<Props> = ({ event }) => {
  return (
    <div className="relative group h-full flex flex-col p-8 border border-[#FFB400]/30 bg-black hover:border-[#FFB400] transition-all duration-300">
      {/* Corner Accents */}
      <div className="absolute top-0 left-0 w-2 h-2 border-t border-l border-[#FFB400]"></div>
      <div className="absolute bottom-0 right-0 w-2 h-2 border-b border-r border-[#FFB400]"></div>

      <div className="flex justify-end items-start mb-6">
        <ArrowUpRight className="text-white/30 group-hover:text-[#FFB400] transition-colors" size={20} />
      </div>

      <h3 className="text-2xl font-bold mb-4 tracking-tight group-hover:text-[#FFB400] transition-colors leading-tight">
        {event.title}
      </h3>
      ...
      <div className="space-y-3 mb-8">
        ...
        {event.location && (
          <div className="flex items-center text-white/50 text-sm font-mono">
            <MapPin size={14} className="mr-2 text-[#FFB400]" />
            {event.location}
          </div>
        )}
      </div>
